STRINGS.ACTIONS.HIRE_PIPSPOOK = "遣灵术"
STRINGS.ACTIONS.LOTUSMUTATION = "变异阿比"
STRINGS.ACTIONS.BREW_UNSTABLE = "不稳定调制"
STRINGS.ACTIONS.BUNK = "假寐"
STRINGS.ACTIONS.MURDER_ABIGAIL = "谋杀阿比盖尔！"
STRINGS.ACTIONS.MOONDIALRITUAL = "变异花"

STRINGS.UI.CRAFTING.RECIPEACTION.BREW_UNSTABLE = "不稳定调制"
STRINGS.UI.CRAFTING.RECIPEACTION.SOULCHILLIFY = "凉拌"
STRINGS.UI.CRAFTING.RECIPEACTION.THRIVING = "蒸！"

STRINGS.NAMES.ABIGAIL_GESTALT_LOTUS = "月灵莲花"
STRINGS.NAMES.UNSTABLE_GHOSTLYELIXIR_REVIVE = "不稳定的恐怖经历"
STRINGS.NAMES.UNSTABLE_GHOSTLYELIXIR_SPEED = "不稳定的强健精油"
STRINGS.NAMES.UNSTABLE_GHOSTLYELIXIR_ATTACK = "不稳定的夜影万金油"
STRINGS.NAMES.UNSTABLE_GHOSTLYELIXIR_RETALIATION = "不稳定的蒸馏复仇"
STRINGS.NAMES.UNSTABLE_GHOSTLYELIXIR_SHIELD = "不稳定的不屈药剂"
STRINGS.NAMES.UNSTABLE_GHOSTLYELIXIR_FASTREGEN = "不稳定的灵魂万灵药"
STRINGS.NAMES.UNSTABLE_GHOSTLYELIXIR_SLOWREGEN = "不稳定的亡者补药"
STRINGS.NAMES.WENDY_LAST_FOOD = "临别香蕉冻"
STRINGS.NAMES.SISTURN_SALTLICK = "姐妹带盐骨灰"
STRINGS.NAMES.SISTURN_FOOD = "糟渣"
STRINGS.NAMES.MURDER_ELIXIR = "谋·诅咒之苦"
STRINGS.NAMES.BIGSPOOK_ALT_NAMES = {
    "大惊吓·泪珠王国的继承者",
    "大惊吓·哀灵的至尊守护者",
    "大惊吓·恐惧的绝世使者",
    "大惊吓·阴影中不灭的光辉",
    "大惊吓·无眠王者的陪伴者",
    "大惊吓·守墓女神的化身",
    "大惊吓·幽冥天使的引导者",
    "大惊吓·临终美丽的天使",
    "大惊吓·陵寝星辰的守护者",
    "大惊吓·墓碑间的命运之子",
    "大惊吓·灵魂哭泣的女王",
    "大惊吓·灰烬世界的公主",
    "大惊吓·惊恐王朝的传道者",
    "大惊吓·绝美噩梦的使者",
    "大惊吓·梦魇中的永恒女神",
    "大惊吓·魂灵引导的星辉",
    "大惊吓·终极惊吓的女王",
    "大惊吓·官方噩梦的使者",
    "大惊吓·无上灵魂的守护神",
    "大惊吓·怨灵之王的传人",
    "大惊吓·黄泉神使的降临者",
    "大惊吓·冥界颤栗的女皇",
    "大惊吓·地府王国的公主",
    "大惊吓·阴司皇宫的使者",
    "大惊吓·鬼界之上的皇后",
    "大惊吓·黄泉边缘的神祇",
    "大惊吓·三途河的天选王者",
    "大惊吓·冥土女神的降临",
    "大惊吓·灵界终极的皇族",
    "大惊吓·复仇之魂的永恒王者",
    "大惊吓·复仇的绝世女王",
    "大惊吓·复仇灵魂的统治者",
    "大惊吓·亡魂复仇的天使",
    "大惊吓·冥界复仇的王者",
    "大惊吓·黄泉深渊的神圣公主",
    "大惊吓·复仇的午夜女王",
    "大惊吓·复仇女神的驾临",
    "大惊吓·复仇的荣耀使者",
    "大惊吓·黄金血脉的冥府公主",
    "大惊吓·不朽荣耀的黄泉贵族",
    "大惊吓·紫晶之冠的黄泉继承者",
    "大惊吓·地狱王座的贵族女神",
    "大惊吓·冥界贵族的荣耀使者",
    "大惊吓·永恒权贵的冥土之主",
    "大惊吓·爬行的泥之公主",
    "大惊吓·石头之心的咆哮公主",
    "大惊吓·粘土之眼的守护者",
    "大惊吓·腐败土壤的王国继承者",
    "大惊吓·地底之城的废墟女王",
    "大惊吓·泥泞之上的飘渺皇后",
    "大惊吓·潮湿的黑暗领主",
    "大惊吓·爬升中的腐朽女神",
    "大惊吓·地下深渊的爬行皇族",
    "大惊吓·腐泥之中的漠视者",
}

STRINGS.RECIPE_DESC.WENDY_LAST_FOOD = "吃了就能与阿比盖尔重逢的临别一餐。"
STRINGS.RECIPE_DESC.UNSTABLE_GHOSTLYELIXIR_REVIVE = "在生活粗暴打断你之后，把你重新拼凑起来。"
STRINGS.RECIPE_DESC.UNSTABLE_GHOSTLYELIXIR_SPEED = "有时候飞快，有时候……算了。"
STRINGS.RECIPE_DESC.UNSTABLE_GHOSTLYELIXIR_ATTACK = "攻击力时高时低，夜视也是时有时无。"
STRINGS.RECIPE_DESC.UNSTABLE_GHOSTLYELIXIR_RETALIATION = "反弹伤害……但数值随缘。"
STRINGS.RECIPE_DESC.UNSTABLE_GHOSTLYELIXIR_SHIELD = "偶尔能挡点伤害。"
STRINGS.RECIPE_DESC.UNSTABLE_GHOSTLYELIXIR_FASTREGEN = "上药时都会痛一下，但后面非常有效果。"
STRINGS.RECIPE_DESC.UNSTABLE_GHOSTLYELIXIR_SLOWREGEN = "缓慢不稳定的恢复，偶尔会受伤。"
STRINGS.RECIPE_DESC.MURDER_ELIXIR = "这回是真的被鬼打了。"

STRINGS.CHARACTERS.GENERIC.DESCRIBE.ABIGAIL_GESTALT_LOTUS = "来自月亮的力量。"
STRINGS.CHARACTERS.WENDY.DESCRIBE.ABIGAIL_GESTALT_LOTUS = "月亮的力量能让阿比变强吗？"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.UNSTABLE_GHOSTLYELIXIR_REVIVE = "在生活粗暴打断你之后，把你重新拼凑起来。"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.UNSTABLE_GHOSTLYELIXIR_SPEED = "有时候飞快，有时候……算了。"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.UNSTABLE_GHOSTLYELIXIR_ATTACK = "攻击力时高时低，夜视也是时有时无。"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.UNSTABLE_GHOSTLYELIXIR_RETALIATION = "反弹伤害……但数值随缘"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.UNSTABLE_GHOSTLYELIXIR_SHIELD = "偶尔能挡点伤害。"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.UNSTABLE_GHOSTLYELIXIR_FASTREGEN = "上药时都会痛一下，但后面非常有效果。"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.UNSTABLE_GHOSTLYELIXIR_SLOWREGEN = "缓慢不稳定的恢复，偶尔会受伤。"
STRINGS.CHARACTERS.WENDY.DESCRIBE.WENDY_LAST_FOOD = {"是阿比最爱吃的吗？", "我不喜欢这个……"}
STRINGS.CHARACTERS.GENERIC.DESCRIBE.WENDY_LAST_FOOD = "看起来怪异又美味的食物。"
STRINGS.CHARACTERS.WENDY.DESCRIBE.SISTURN_SALTLICK = "这是阿比盖尔带盐的骨灰……"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.SISTURN_SALTLICK = "一点盐分能让哀悼变得更持久。"
STRINGS.CHARACTERS.WENDY.DESCRIBE.SISTURN_FOOD = "也许能喂饱饥饿的野兽……或熬成一锅汤，用悲伤和阿比盖尔的骨灰调味。"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.SISTURN_FOOD = "看起来能勉强给粗食动物们糊口……或者做锅底料。"
STRINGS.CHARACTERS.WENDY.DESCRIBE.MURDER_ELIXIR = "阿比盖尔，我们的灵药正在蒸蒸日上哦。"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.MURDER_ELIXIR = "这灵药看起来还行……大概吧。"

STRINGS.CHARACTERS.GENERIC.ANNOUNCE_SMALLGHOST_LEAVE = {
    "可怜的灵魂，离开吧。",
    "现在轮到你来帮助我，但我却只想念她。",
    "请帮我找一些能填补空缺的东西，不要是玩具。",
    "快走吧，我无法再承受这份孤独。",
    "你也应该为我做点什么，我的姐妹已经不在了。",
    "我并没有抛弃你，只是失去了最亲的她……",
    "我希望能再次听到她的声音，但这似乎不可能。",
    "我只希望你能帮我找到一些她的回忆。",
}
STRINGS.CHARACTERS.GENERIC.ANNOUNCE_SMALLGHOST_BACK = {
    "可怜的灵魂，你带回来了什么？",
    "你找到我失去的东西了吗？",
    "小惊吓，你能带回我的最爱吗？",
    "你带回来的东西能让我不再孤单吗？",
    "我真的很想念我的姐妹，她能回来吗？",
    "你能不能带回一些她的回忆，让我不再孤独？",
    "可否告诉我，她的灵魂在哪里？",
    "我只想再次感受她的温暖，哪怕是片刻。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_GET_OFF_BUNKER = {
    "请不要让我离开，我还没准备好。",
    "我不想被赶走，这里是我刚装饰过的。",
    "再给我一点时间，我还没见到阿比。",
    "再等等我一会儿，我还没见到阿比盖尔呢。",
    "我好像见到阿比盖尔了。",
    "尽管阴暗潮湿，我还是活下来了。",
}
STRINGS.CHARACTERS.GENERIC.ANNOUNCE_GET_OFF_BUNKER = {
    "那些恶鬼又回来了。",
    "呆在潮湿又阴暗的地方真让人反感。",
    "这里又潮湿又阴暗。",
    "阴暗潮湿的坟墓令人不适",
}
STRINGS.CHARACTERS.GENERIC.ANNOUNCE_PARTING = {
    "一口下肚，灵魂便可飘离。",
    "吃下它，我将轻轻离开这具躯壳。",
    "这最后的滋味，送我灵魂出窍。",
    "饱餐一顿，便能与肉体诀别。",
    "只需一餐，我便能在空中游荡。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_PARTING = {
    "吃下去，我就能见到阿比盖尔了……",
    "吃下它，我就能回到阿比盖尔身边。",
    "再吃一口，灵魂就能与阿比同在。",
    "我们曾经最爱吃的食物。",
}
STRINGS.CHARACTERS.GENERIC.ANNOUNCE_NOT_PARTING = {
    "温蒂不在，没人能把我变成鬼魂！",
    "安全了，温蒂没学会那个可怕的法术。",
    "吃了也不会死！谢天谢地，温蒂没在。",
    "没有温蒂，没有复仇，只有平静。",
    "温蒂没在附近，我的灵魂暂时安全。",
    "没人引导灵魂，我还能活得好好的。",
    "温蒂不在场，我不用担心灵魂出窍。",
    "这是安全的食物，我的灵魂还牢牢地在身体里。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_NOT_PARTING = {
    "我还不能踏上阿比盖尔的足迹。",
    "我还不知道如何灵魂出窍。",
    "没有鬼魂复仇的仪式，我只是一个孤独的凡人。",
    "我变不成鬼魂，连见她一面都不能。",
    "如果我知道怎么做，我早就跟上她了……",
    "灵魂还困在这具身体里，我无能为力。",
    "阿比盖尔……我还追不上你。",
    "我只能等待命运带我去她身边。",
    "我不知道那条鬼魂之路怎么走。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_LOST_GHOST_POWER = {
    "再次失去了鬼魂的力量和勇气。",
    "鬼魂复仇给我的力量消失了。",
    "阿比盖尔……你还在看着我吗？",
    "没有你，我连复仇都做不到……",
    "我又孤单一人了，阿比盖尔。",
    "她的力量离开了我，温柔也一并消散了。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_ASHES_LICK = {
    "如果骨灰有味道，我希望它至少加点盐……",
    "原来“骨灰级粉丝”的字面意思是这样……对不起，我开玩笑的。",
    "我猜阿比一定没想到，她的最终归宿会是牛饲料……",
    "阿比，我发誓这是最后一次……除非明天牛还想吃。",
    "如果牛死了，算不算阿比盖尔的转世？……算了，当我没说。",
    "阿比盖尔……你会原谅我吗？",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_NOT_FLESHEILD = {
    "阿比盖尔不需要肉盾，她需要的是入土为安。",
    "尸体上贴瘦肉不会变成战士，只会吸引秃鹫。",
    "如果肉盾能保护她，那阿比就不会死了，对吧？",
    "我没掌握让阿比盖尔附上肉盾的技巧。",
    "为鬼魂加肉盾听起来像某种祭司的恶趣味。",
    "肉盾只是让死亡迟到，不是取消预约。",
    "我不太擅长瘦肉骨灰罐这个技能树。",
}
STRINGS.CHARACTERS.GENERIC.ANNOUNCE_NOT_FLESHEILD = {
    "肉盾？听起来像是自找麻烦，我不想被牵连。",
    "肉盾根本不能解决问题，只会把麻烦推给别人。",
    "我不想让自己和尸体扯上关系，太恶心了。",
}
STRINGS.CHARACTERS.GENERIC.ANNOUNCE_NOT_ENOUGH_FLESH = {
    "肉块不足可真是个问题！",
    "我需要更多肉块才能完成这个任务。",
    "没有足够的肉块。",
    "这真是个麻烦，我不喜欢缺少资源。",
    "我希望能找到更多肉块。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_GHOST_NOT_SUMMONED = {
    "阿比盖尔，在哪里？",
    "阿比盖尔，我需要你。",
    "阿比盖尔，快出来。",
    "阿比盖尔不在这里，难道她又消失了？",
    "阿比盖尔，别再躲了！",
    "阿比盖尔，我感到孤单。",
    "我听不到阿比盖尔的声音……太安静了。",
    "阿比盖尔的灵魂如今也遗弃了我吗？",
    "连阿比盖尔都舍我而去。",
    "风里没有阿比盖尔的回音，只有寂静。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_GHOST_MUTATION_COOLDOWN = {
    "阿比盖尔需要时间回应。",
    "阿比盖尔需要时间恢复。",
    "阿比盖尔还没准备好。",
    "阿比盖尔的灵魂还在休息。",
    "阿比盖尔的力量尚未恢复。",
    "阿比盖尔暂时无法再变异。",
    "变异也暂时带走了她的力气。",
    "连死者也有疲惫之时。",
    "死亡并不慷慨，它也设下等待的门槛。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_FLOWER_MUTATION_COOLDOWN = {
    "月晷尚未回应我的呼唤。",
    "仪式的回响还在沉眠。",
    "月光拒绝了再次指引我。",
    "冷却的时刻尚未过去。",
    "连月亮也需休息。",
    "时间还未织好第二道花环。",
    "月亮之力尚未归位。",
    "虚影之门仍紧闭。",
    "命运不容催促，它静候星辰归位。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_HAUNT_COOLDOWN = {
    "她刚刚被灌注鲜血，灵魂尚未稳固。",
    "灵与血才交汇，无法再次唤起她。",
    "鲜血扰乱了她与彼岸的连结。",
    "灵魂的通道仍在凝合中。",
    "血液温热，而她仍在冷却。",
    "刚才的共鸣太剧烈……她需要静默。",
    "血液换新，灵魂却因此迟疑。",
    "她在血与死之间踟蹰，尚未归来。",
    "连死者也畏惧鲜血的喧嚣。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_HAUNT_READY = {
    "血与魂已协调。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_NEED_APART = {
    "阿比盖尔还没准备贴贴。",
    "阿比盖尔太累了，附身不了。",
    "阿比盖尔现在不行。",
    "阿比盖尔要先歇一会。",
    "我们得分开一阵子。",
    "现在不能附身。",
    "阿比盖尔说别急。",
    "阿比盖尔要休息一下。",
    "阿比盖尔刚刚才分开，别逼她。",
    "阿比盖尔还需要点时间。",
    "鬼魂也需要点时间。",
    "阿比盖尔暂时不想和我贴贴。",
}
STRINGS.CHARACTERS.WENDY.ANNOUNCE_NO_FLOWER_INVENTORY = {
    "我真的需要阿比盖尔之花……没有它，我做不到。",
}
STRINGS.SISTURNFILTER =
{
    UI = {
        BLESSED_FLESHED = "受佑瘦肉"
    }
}

if GetModConfigData("PipspookSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_SMALLGHOST_3_DESC = STRINGS.SKILLTREE.WENDY.WENDY_SMALLGHOST_3_DESC ..
    "\n可以反过来给小惊吓安排任务，让她帮找寻稀有物品。"
end
if GetModConfigData("ElixirSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_POTION_YIELD_DESC = STRINGS.SKILLTREE.WENDY.WENDY_POTION_YIELD_DESC ..
    "\n可以制造不稳定的高浓度灵药，对群体使用。"
end
if GetModConfigData("SisturnSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_SISTURN_3_DESC = STRINGS.SKILLTREE.WENDY.WENDY_SISTURN_3_DESC ..
    "\n可以将瘦肉投入骨灰罐，转化为阿比盖尔的肉盾值，并产出带盐骨灰与糟渣。"
end
if GetModConfigData("CraftingSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_GHOSTFLOWER_GRAVE_DESC = STRINGS.SKILLTREE.WENDY.WENDY_GHOSTFLOWER_GRAVE_DESC ..
    "\n温蒂每次从多年生植物祭坛复活时，阿比盖尔因对温蒂的保护欲增强而变得更强。"
end
if GetModConfigData("PartingSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_AVENGING_GHOST_DESC = STRINGS.SKILLTREE.WENDY.WENDY_AVENGING_GHOST_DESC ..
    "\n可以制作食用后灵魂出窍的临别香蕉冻；灵魂出窍期间碰撞生物可积蓄鬼魂复仇之力，鬼魂复仇之力可以提高攻击力到1.0倍率。"
end
if GetModConfigData("BunkerSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_MAKEGRAVEMOUNDS_DESC = STRINGS.SKILLTREE.WENDY.WENDY_MAKEGRAVEMOUNDS_DESC ..
    "\n可以钻进装饰好的墓碑躲起来，让大惊吓保护。"
end
if GetModConfigData("CallingSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_GHOSTCOMMAND_3_DESC = STRINGS.SKILLTREE.WENDY.WENDY_GHOSTCOMMAND_3_DESC ..
    "\n阿比盖尔的作祟指令可以对温蒂使用，交换当前血量百分比。"
end
if GetModConfigData("LunarSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_LUNAR_3_DESC = STRINGS.SKILLTREE.WENDY.WENDY_LUNAR_3_DESC ..
    "满月全天，可以用月晷将阿比盖尔之花转化为月灵莲花；月灵莲花可以切换阿比盖尔的鬼魂或虚影形态。"
end
if GetModConfigData("ShadowSetting") then
    STRINGS.SKILLTREE.WENDY.WENDY_SHADOW_3_DESC = STRINGS.SKILLTREE.WENDY.WENDY_SHADOW_3_DESC ..
    "\n用灵药谋杀阿比盖尔，使温蒂被鬼灵缠身，继承阿比盖尔的范围伤害与易伤增益。"
end